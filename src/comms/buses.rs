...
pub struct ThreadedMessenger<T: Message> {
    dirin: Vec<Arc<ArrayQueue<T>>>,
    dirout: Vec<Arc<ArrayQueue<T>>>,
    broadcaster: Arc<ArrayQueue<T>>,
    slots: usize,
    pub capacity: usize,
...
impl<T: Message> ThreadedMessenger<T> {
    ...
    pub fn new(user_count: usize, slots: usize) -> Result<Self, MesoError> {
        ...
        let broadcaster = Arc::new(ArrayQueue::new(slots));
    ...
    pub fn get_user(&mut self) -> Result<ThreadedMessengerUser<T>, MesoError> {
        ...
        let subscriber = self.broadcaster.clone();
...
//     let mut world = ThreadedMessenger::<StressMessage>::new(NUM_USERS, SLOTS).unwrap();
