use std::{alloc::{alloc, dealloc, Layout}, collections::BTreeSet, ptr::{self, null_mut}, usize};
...
impl Default for AllocKind {
    fn default() -> Self {
        Self::Arena { end: u64::MAX, counter: 0 }
    }
}
...
impl Default for MetaLog {
    fn default() -> Self {
        Self { size: 0, align: 8, offset: 0, alloc_idx: usize::MAX }
    }
}
...
impl Default for LogState {
    fn default() -> Self {
        Self { state: null_mut(), meta: MetaLog::default(), time: u64::MAX }
    }
}
...
impl Journal {
    ...
    pub fn write<T: Pod + Zeroable + 'static>(&mut self, state: T, time: u64, horizon: Option<u64>) {
        ...
        if size > self.size || self.arena.is_null() {
            ...
            let log_state = LogState { state: allocation.ptr, meta, time };
        ...
        unsafe {
            ...
            self.state = LogState { state: ptr, meta, time }
        }

        self.offset = end;
        self.current_writes.push(self.state);
        let mut update_start = false;
        match &mut self.allocations[self.active_id].kind {
            AllocKind::Arena { end, counter } => {
                if *counter == 0 {
                    update_start = true;
                }
                *counter += 1;
        ...
        for (i, log) in self.current_writes.iter().enumerate().rev() {
            if log.time > time {
                ...
                if self.allocations[log.meta.alloc_idx].active {
                    if let AllocKind::Arena { counter, .. } = &mut self.allocations[log.meta.alloc_idx].kind {
...
mod tests {
    ...
    fn test_horizon_garbage_collection() {
        ...
        assert!(all.iter().all(|(s, t)| *t >= 1000));
    ...
    fn test_deterministic_replay() {
        ...
        let checkpoint = journal.read_state::<SimulationState>().unwrap().clone();
    ...
    fn test_network_rollback_scenario() {
        ...
        for i in 6..10 {
            let state = SmallState { x: i, y: i as f32 + 0.5 };
            journal.write(state, i as u64 * 100, None);
        }
    ...
    fn test_type_punning_safety() {
        ...
        let a = A { x: 0xDEADBEEF, y: 0xCAFEBABE };
